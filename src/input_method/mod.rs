//! The emitting half of SUIS: turn a device into an input method other clients can use.
//!
//! Hold one [`Node<InputMethod<S>>`] and the query, the handler cache, the capture
//! bookkeeping, the gained/updated/left edges and the per-handler transforms are all
//! handled for you. Your device only implements [`InputMethodHelper`]: who gets input and
//! in what order, its shape in its own space, and what the datamap says.

mod localize;
pub use localize::localize;
mod query_cache;
pub use query_cache::*;

use crate::drop_handlers::AbortOnDrop;
use gluon::{Context, Handler, Liveness, Node, RefExt};
use stardust_xr_fusion::{
	Result,
	client::{Client, ClientHandler, FrameInfo},
	fields::{FieldInterface, FieldRef, FieldSample, RayMarchResult},
	query::{InterfaceDependency, QueryableId},
	spatial::{SpatialInterface, SpatialRef},
	spatial_query::{
		BeamQuery, BeamQueryHandle, BeamQueryHandler, Point, PointsQuery, PointsQueryHandle,
		PointsQueryHandler, ZoneQuery, ZoneQueryHandle, ZoneQueryHandler,
	},
	suis::{
		DatamapData, InputDataType, InputHandler as InputHandlerProxy,
		InputMethod as InputMethodProxy, InputMethodCapture, InputMethodCaptureHandler,
		InputMethodHandler, SemanticData, SpatialData,
	},
	types::{Timestamp, Vec2F, Vec3F},
};
use std::{
	collections::{HashMap, HashSet},
	fmt::{Debug, Formatter},
	future::Future,
	ops::Deref,
	sync::{Arc, Mutex, OnceLock},
};
use tokio::sync::{RwLock, broadcast, mpsc};

fn handler_dependency() -> InterfaceDependency {
	InterfaceDependency {
		id: InputHandlerProxy::QUERY_INTERFACE.to_string(),
		optional: false,
	}
}

/// The device behind an [`InputMethod`].
///
/// Describe the device in its own space and [`localize`] puts it in each handler's space,
/// filling in every distance on the way. Everything is async because a client can't touch a
/// handler's field or spatial directly, both are round trips to the server.
pub trait InputMethodHelper: Send + Sync + 'static {
	/// what the query reports about each handler: [`RayMarchResult`] for a beam,
	/// [`FieldSample`] for points and zones
	type QueryValue: Send + Sync + 'static;

	/// which handlers get input this cycle, closest first, and which capture requester wins
	///
	/// runs while the cache is locked for reading, so the query can't add or drop handlers
	/// until it finishes, keep any sampling in here to what the ordering actually needs
	fn order_handlers_and_captures(
		&self,
		handlers: &HashMap<QueryableId, CachedHandler<Self::QueryValue>>,
		capture_requests: &HashSet<InputHandlerProxy>,
	) -> impl Future<Output = (Vec<InputHandlerProxy>, Option<InputHandlerProxy>)> + Send + Sync;

	/// this method's shape in its own space, None to send nothing this cycle
	///
	/// `time` is when the input is for, which devices that can locate themselves in the past
	/// (an OpenXR space, say) should honor, the rest can ignore
	fn input_data(
		&self,
		time: Timestamp,
	) -> impl Future<Output = Option<InputDataType>> + Send + Sync;

	fn datamap(&self) -> impl Future<Output = HashMap<String, DatamapData>> + Send + Sync;
}

#[derive(Default)]
struct ActiveTracker {
	active: HashSet<InputHandlerProxy>,
}
impl ActiveTracker {
	fn update(
		&mut self,
		new: HashSet<InputHandlerProxy>,
	) -> (HashSet<InputHandlerProxy>, HashSet<InputHandlerProxy>) {
		let added = new.difference(&self.active).cloned().collect();
		let removed = self.active.difference(&new).cloned().collect();
		self.active = new;
		(added, removed)
	}
}

#[derive(Debug, Handler)]
struct CaptureGuard {
	handler: InputHandlerProxy,
	release_tx: mpsc::UnboundedSender<InputHandlerProxy>,
}
impl InputMethodCaptureHandler for CaptureGuard {}
impl Drop for CaptureGuard {
	fn drop(&mut self) {
		let _ = self.release_tx.send(self.handler.clone());
	}
}

#[derive(Handler)]
pub struct InputMethod<S: InputMethodHelper> {
	helper: S,
	cache: QueryCache<S::QueryValue>,
	spatial: SpatialRef,
	spatials: SpatialInterface,
	fields: FieldInterface,
	active_capture: RwLock<Option<InputHandlerProxy>>,
	release_tx: mpsc::UnboundedSender<InputHandlerProxy>,
	release_rx: Mutex<mpsc::UnboundedReceiver<InputHandlerProxy>>,
	tracker: Mutex<ActiveTracker>,
	// bare proxy, not a LocalRef: a LocalRef here would be an Arc cycle back into this handler
	proxy: OnceLock<InputMethodProxy>,
	// the query lives exactly as long as its handler node, so holding it here is what makes
	// dropping the method drop the query
	query: OnceLock<Box<dyn Send + Sync>>,
}
impl<S: InputMethodHelper> Debug for InputMethod<S> {
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("InputMethod").finish_non_exhaustive()
	}
}
impl<S: InputMethodHelper> Deref for InputMethod<S> {
	type Target = S;

	fn deref(&self) -> &S {
		&self.helper
	}
}

impl<S: InputMethodHelper> InputMethod<S> {
	fn build(
		helper: S,
		cache: QueryCache<S::QueryValue>,
		spatial: SpatialRef,
		spatials: SpatialInterface,
		fields: FieldInterface,
	) -> Self {
		let (release_tx, release_rx) = mpsc::unbounded_channel();
		Self {
			helper,
			cache,
			spatial,
			spatials,
			fields,
			active_capture: RwLock::new(None),
			release_tx,
			release_rx: Mutex::new(release_rx),
			tracker: Mutex::default(),
			proxy: OnceLock::new(),
			query: OnceLock::new(),
		}
	}

	fn finish<H: ClientHandler>(
		client: &Client<H>,
		helper: S,
		cache: QueryCache<S::QueryValue>,
		spatial: SpatialRef,
		query: impl Send + Sync + 'static,
	) -> Result<(Node<Self>, InputMethodProxy)> {
		let method = Self::build(
			helper,
			cache,
			spatial,
			client.spatial_interface().clone(),
			client.field_interface().clone(),
		);
		let (node, method) = InputMethodProxy::new_node(method)?;
		let proxy = method.into_proxy();
		let _ = node.proxy.set(proxy.clone());
		let _ = node.query.set(Box::new(query));
		Ok((node, proxy))
	}

	pub fn helper(&self) -> &S {
		&self.helper
	}

	pub fn cache(&self) -> &QueryCache<S::QueryValue> {
		&self.cache
	}

	/// the space this method lives in: what the query is relative to, and what
	/// [`InputMethodHelper::input_data`] is relative to
	pub fn spatial(&self) -> &SpatialRef {
		&self.spatial
	}

	/// the handler currently winning the capture, if any
	pub async fn active_capture(&self) -> Option<InputHandlerProxy> {
		self.active_capture.read().await.clone()
	}

	/// force-release the active capture, as if the capturing client dropped its guard
	///
	/// takes effect on the next [`Self::send`], which is what drains the release channel
	pub async fn stop_active_capture(&self) {
		if let Some(handler) = self.active_capture.read().await.clone() {
			let _ = self.release_tx.send(handler);
		}
	}

	async fn grant_capture(&self, handler: InputHandlerProxy) -> Option<InputMethodCapture> {
		// Hold the handler write lock across the membership check and the capture_requests
		// insert so on_left, which checks capture_requests under the same lock, can't slip in
		// and drop this handler's entry on a concurrent field exit.
		{
			let handlers = self.cache.handlers_mut().await;
			if !handlers.values().any(|e| e.handler == handler) {
				return None;
			}
			self.cache
				.capture_requests_mut()
				.await
				.insert(handler.clone());
		}
		let capture = InputMethodCapture::new_service(CaptureGuard {
			handler,
			release_tx: self.release_tx.clone(),
		})
		.ok()?;
		Some(capture.into_proxy())
	}

	async fn drain_released_captures(&self) {
		let released: Vec<InputHandlerProxy> = {
			let mut rx = self.release_rx.lock().unwrap();
			std::iter::from_fn(|| rx.try_recv().ok()).collect()
		};
		for handler in released {
			self.cache.capture_requests_mut().await.remove(&handler);
			self.cache
				.handlers_mut()
				.await
				.retain(|_, e| !(e.left_query && e.handler == handler));
			let mut capture = self.active_capture.write().await;
			if capture.as_ref() == Some(&handler) {
				capture.take();
			}
		}
	}

	/// sweep handlers whose client died without the query ever saying they left
	async fn drain_dead_handlers(&self) {
		if !self
			.cache
			.handlers()
			.await
			.values()
			.any(|e| !e.handler.alive())
		{
			return;
		}
		let mut capture_requests = self.cache.capture_requests_mut().await;
		self.cache.handlers_mut().await.retain(|_, e| {
			let alive = e.handler.alive();
			if !alive {
				capture_requests.remove(&e.handler);
			}
			alive
		});
	}

	/// one dispatch cycle: work out who gets what and send it
	///
	/// call this whenever the device has fresh data, or let
	/// [`spawn_frame_driver`] run it once a frame
	pub async fn send(&self, time: Timestamp) {
		let Some(method) = self.proxy.get() else {
			return;
		};
		self.drain_released_captures().await;
		self.drain_dead_handlers().await;

		// a handler whose get_spatial is still in flight is skipped rather than kept in
		// order, so its first real send is always an input_gained
		let (capture, targets) = {
			let handlers = self.cache.handlers().await;
			let capture_requests = self.cache.capture_requests().await.clone();
			let (order, capture) = self
				.helper
				.order_handlers_and_captures(&handlers, &capture_requests)
				.await;
			let targets: Vec<(InputHandlerProxy, SpatialRef, FieldRef)> = order
				.into_iter()
				.filter_map(|handler| {
					let entry = handlers.values().find(|e| e.handler == handler)?;
					Some((handler, entry.spatial.clone()?, entry.field.clone()))
				})
				.collect();
			(capture, targets)
		};
		*self.active_capture.write().await = capture.clone();

		let (datamap, input) = futures::join!(self.helper.datamap(), self.helper.input_data(time));

		// an untracked device dispatches to nobody rather than returning early, so whoever was
		// active still gets their input_left
		let mut dispatch: Vec<(InputHandlerProxy, SpatialData, SemanticData)> = Vec::new();
		if let Some(input) = input {
			// localizing is a round trip per handler at minimum, so they all go out at once
			// rather than costing one each
			let spatial_data =
				futures::future::join_all(targets.iter().map(|(_, spatial, field)| {
					localize(
						&self.spatials,
						&self.fields,
						&self.spatial,
						spatial,
						field,
						&input,
					)
				}))
				.await;

			dispatch = targets
				.into_iter()
				.zip(spatial_data)
				.filter_map(|((handler, ..), spatial)| Some((handler, spatial?)))
				.enumerate()
				.map(|(order, (handler, spatial))| {
					let semantic = SemanticData {
						datamap: datamap.clone(),
						order: order as u32,
						captured: capture.as_ref() == Some(&handler),
					};
					(handler, spatial, semantic)
				})
				.collect();
		}

		let (added, removed) = self.tracker.lock().unwrap().update(
			dispatch
				.iter()
				.map(|(handler, ..)| handler.clone())
				.collect(),
		);

		for (handler, spatial, semantic) in dispatch {
			let _ = if added.contains(&handler) {
				handler.input_gained(method.clone(), time, spatial, semantic)
			} else {
				handler.input_updated(method.clone(), time, spatial, semantic)
			};
		}
		for handler in removed {
			let _ = handler.input_left(method.clone(), time);
		}
	}
}

impl<S: InputMethodHelper<QueryValue = RayMarchResult>> InputMethod<S> {
	pub async fn new_beam<H: ClientHandler>(
		client: &Client<H>,
		helper: S,
		spatial: SpatialRef,
		origin: Vec3F,
		direction: Vec3F,
		max_length: f32,
	) -> Result<(Node<Self>, InputMethodProxy, BeamQueryHandle)> {
		let cache = QueryCache::default();
		let (query_node, query) = BeamQueryHandler::new_node(BeamQueryCache(cache.clone()))?;
		let handle = client
			.spatial_query_interface()
			.beam_query(BeamQuery {
				handler: query.into_proxy(),
				interfaces: vec![handler_dependency()],
				reference_spatial: spatial.clone(),
				origin,
				direction,
				max_length,
			})
			.await??;
		let (node, proxy) = Self::finish(client, helper, cache, spatial, query_node)?;
		Ok((node, proxy, handle))
	}
}

impl<S: InputMethodHelper<QueryValue = FieldSample>> InputMethod<S> {
	pub async fn new_points<H: ClientHandler>(
		client: &Client<H>,
		helper: S,
		spatial: SpatialRef,
		points: Vec<Point>,
	) -> Result<(Node<Self>, InputMethodProxy, PointsQueryHandle)> {
		let cache = QueryCache::default();
		let (query_node, query) = PointsQueryHandler::new_node(PointsQueryCache(cache.clone()))?;
		let handle = client
			.spatial_query_interface()
			.points_query(PointsQuery {
				handler: query.into_proxy(),
				interfaces: vec![handler_dependency()],
				reference_spatial: spatial.clone(),
				points,
			})
			.await??;
		let (node, proxy) = Self::finish(client, helper, cache, spatial, query_node)?;
		Ok((node, proxy, handle))
	}

	pub async fn new_zone<H: ClientHandler>(
		client: &Client<H>,
		helper: S,
		spatial: SpatialRef,
		zone_field: FieldRef,
		margin: f32,
	) -> Result<(Node<Self>, InputMethodProxy, ZoneQueryHandle)> {
		let cache = QueryCache::default();
		let (query_node, query) = ZoneQueryHandler::new_node(ZoneQueryCache(cache.clone()))?;
		let handle = client
			.spatial_query_interface()
			.zone_query(ZoneQuery {
				handler: query.into_proxy(),
				interfaces: vec![handler_dependency()],
				zone_field,
				margin,
			})
			.await??;
		let (node, proxy) = Self::finish(client, helper, cache, spatial, query_node)?;
		Ok((node, proxy, handle))
	}
}

impl<S: InputMethodHelper> InputMethodHandler for InputMethod<S> {
	async fn request_capture(
		&self,
		_ctx: Context,
		handler: InputHandlerProxy,
	) -> Option<InputMethodCapture> {
		self.grant_capture(handler).await
	}

	async fn get_spatial_data(
		&self,
		_ctx: Context,
		handler: InputHandlerProxy,
		time: Timestamp,
	) -> Option<SpatialData> {
		if self
			.active_capture
			.read()
			.await
			.as_ref()
			.is_some_and(|c| c != &handler)
		{
			return None;
		}
		let cached = self
			.cache
			.handlers()
			.await
			.values()
			.find(|e| e.handler == handler)
			.and_then(|e| Some((e.spatial.clone()?, e.field.clone())));
		// a handler outside the query can still ask, it just costs two round trips
		let (spatial, field) = match cached {
			Some(pair) => pair,
			None => (
				handler.get_spatial().await.ok()?,
				handler.get_field().await.ok()?,
			),
		};
		let input = self.helper.input_data(time).await?;
		localize(
			&self.spatials,
			&self.fields,
			&self.spatial,
			&spatial,
			&field,
			&input,
		)
		.await
	}
}

/// drive an input method once per frame
///
/// devices that poll faster than the display should just call [`InputMethod::send`]
/// themselves, that's the whole point of it being async
pub fn spawn_frame_driver<S: InputMethodHelper>(
	node: &Node<InputMethod<S>>,
	mut frames: broadcast::Receiver<FrameInfo>,
) -> AbortOnDrop {
	let method = Arc::downgrade(node.handler());
	tokio::spawn(async move {
		while let Ok(info) = frames.recv().await {
			let Some(method) = method.upgrade() else {
				break;
			};
			method.send(info.predicted_display_time).await;
		}
	})
	.into()
}

/// Builds the datamap an [`InputMethodHelper`] hands back.
#[derive(Debug, Default, Clone)]
pub struct DatamapBuilder(HashMap<String, DatamapData>);
impl DatamapBuilder {
	pub fn bool(mut self, key: &str, value: bool) -> Self {
		self.0.insert(key.to_string(), DatamapData::Bool { value });
		self
	}
	pub fn f32(mut self, key: &str, value: f32) -> Self {
		self.0.insert(key.to_string(), DatamapData::Float { value });
		self
	}
	pub fn vec2(mut self, key: &str, value: impl Into<Vec2F>) -> Self {
		self.0.insert(
			key.to_string(),
			DatamapData::Vec2 {
				value: value.into(),
			},
		);
		self
	}
	pub fn vec3(mut self, key: &str, value: impl Into<Vec3F>) -> Self {
		self.0.insert(
			key.to_string(),
			DatamapData::Vec3 {
				value: value.into(),
			},
		);
		self
	}
	pub fn string(mut self, key: &str, value: impl Into<String>) -> Self {
		self.0.insert(
			key.to_string(),
			DatamapData::String {
				value: value.into(),
			},
		);
		self
	}
	pub fn build(self) -> HashMap<String, DatamapData> {
		self.0
	}
}
