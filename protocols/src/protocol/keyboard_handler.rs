...
pub trait KeyboardHandlerHandler: gluon::Handler + Send + Sync + 'static {
	...
	fn to_node(
		self,
	) -> Result<(gluon::Node<Self>, gluon::LocalRef<KeyboardHandler, Self>), gluon::NodeError>
	where
		Self: Sized,
	{
		use gluon::RefExt;
	...
	fn to_service(self) -> Result<gluon::LocalRef<KeyboardHandler, Self>, gluon::NodeError>
	where
		Self: Sized,
	{
		use gluon::RefExt;
